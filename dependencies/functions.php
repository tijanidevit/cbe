...
try {
    ...
    if (isset($_SESSION['verified'])) {
        ...
        echo displayError("<h3 class='text-center'>Opps! The server couldn't verify this software.</h3>");
        exit();
    }
} catch (PDOException $e) {
    echo displayError("<h3 class='text-center'>Opps! The server couldn't connect to database.</h3>");
    exit();
}
...
{
    return '<div class="msgElement alert alert-danger">' . $message . '</div>';
}

function displayWarning($message)
{
    return '<div class="msgElement alert alert-warning">' . $message . '</div>';
}

function displaySuccess($message)
{
    return '<div class="msgElement alert alert-success">' . $message . '</div>';
}

function displayInformation($message)
{
    return '<div class="msgElement alert alert-info">' . $message . '</div>';
}
...
{
    global $conn;
    $query = $conn->prepare("SELECT $c FROM $a WHERE $b='$d'");
    $query->execute();
