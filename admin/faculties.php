...
<head>
        <meta charset="utf-8" />
        <title>Faculties  - <?php echo $config['school_name']; ?></title>
 ...
 <div class="row">
                        ...
                        <div class="card-box">
                           ...
                           <div class="row" style="display: none;" id="new-form">
                               <div class="col-md-6 offset-md-3">
                                    <form method="post" action="actions/new-faculty.php">
                    ...
                    <div class="col-12">
                        <div class="card-box table-responsive">
                             <?php 
                              $query = $conn->prepare("SELECT * FROM faculty WHERE active='1' ORDER BY faculty_name ASC");
                              $query->execute();
                              if (($query->rowCount()) == 0) {
                                ...
                                <table id="responsive-datatable" class="table table-bordered table-bordered dt-responsive norap">
                                    ...
                                    while ($row= $query->fetch(PDO::FETCH_ASSOC)) { $i++; ?>
                                        <tr>
                                            ...
                                            <td><?php echo getRowCount('departments','faculty_id',$row['faculty_id']); ?></td>
                                            <td>                                                
                                                <a href="#edit-modal<?php echo $row['faculty_id']; ?>" data-animation="door" data-plugin="custommodal" data-overlayColor="#36404a" class="btn btn-warning show-faculty-edit" target-id="<?php echo $row['faculty_id']; ?>" target-name="<?php echo $row['faculty_name']; ?>"><i class="fa fa-edit"></i></a>


                                                <a href="actions/trash-faculty.php?q=<?php echo encrypt($row['faculty_id']); ?>" class="btn btn-danger trash-button2"><i class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                        <!-- Modal -->
        <div id="edit-modal<?php echo $row['faculty_id']; ?>" class="modal-demo">
                ...
                <div class="custom-modal-text">
                        <div class="row">
                               <div class="col-md-6 offset-md-3">
                                    <form method="post" action="actions/edit-faculty.php">
                                <div class="form-group">
                                    <label class="label">Faculty Name</label>
                                    <input type="text" name="faculty_name" class="form-control faculty_name" placeholder="Name" required="required" value="<?php echo $row['faculty_name']; ?>" /> 
                                    <input type="hidden" class="faculty_id" value="<?php echo $row['faculty_id']; ?>" name="faculty_id" />
                                </div>
                                <div class="form-group text-center">
                                   <button class="btn bg-theme-colored update-faculty" type="submit" name="update-faculty">Update</button>
                                </div>
                            </form>
                               </div>
                           </div>
